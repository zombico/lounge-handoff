...
#003 the world mesh is LIT by MojuloSun + the sky atmosphere through M_MojuloLit — sun patches through the panes, a blue sky in the windows, no "Preview" stamp on the shadows (reference look: blender-bake.mjs --render); a score `sky` preset sets the rig (night: a dim cool moon, a dark atmosphere, height fog, the lamp lights carrying the scene) and the fallback ground is a visible matte plane, never a black void
...
#104 contract — declares T0 (coherent); for T1 (baked): out_normals — 99.9% of faces carry outNormal (a bake reads winding from it)
#105 textures_carried ×1 — surface/atlas textures travel inside the GLB (Interchange imports them as base-colour maps — pinned at the first machine gate); the web build is the reference look
#106 promoted_ground — implicit runtime ground plane promoted by the importer — the collider AABBs are obstacle hulls only, never the floor
#107 entity_markers — entities that baked no mesh (glyph/primitive bodies — export-side gap) get placeholder markers; the web build is the reference look
#108 cameras_data_only ×2 — authored camera framings ride score.json as data; the walker camera is the play view
